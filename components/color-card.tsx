...
export function ColorCard({ color, index, onChange, onSelect }: ColorCardProps) {
  const [copied, setCopied] = useState(false)
  const [displayFormat, setDisplayFormat] = useState<"hex" | "rgb" | "hsl" | "css" | "tailwind">("hex")
  ...
  } else if (displayFormat === "css") {
    displayValue = `var(--color-${index + 1})`
  } else if (displayFormat === "tailwind") {
  ...
  const handleFormatChange = (e: React.MouseEvent) => {
    ...
    else if (displayFormat === "css") setDisplayFormat("tailwind")
